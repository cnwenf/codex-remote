...
## 0.5.33：后台监控恢复修复与发布 — 2026-09-09

本轮按用户最新要求只处理持续出现的“后台监控异常”及发布，图片发送问题明确延期。修复提交与发布标签固定在 `1347bce5f54900feb87178c71fcb1dc4eb9d8e8f`，未改图片生产代码；现有 Mac Gateway 0.5.32 可继续使用，没有更新 Mac 安装或重启 Codex Desktop。

### 已复现原因及修复

- 原生旧实现的单线程状态请求没有整体截止时间；旧响应持续缓慢返回时，手动重试仍排在旧请求后。修改前，真实 Android instrumentation 的重试 3 秒恢复断言及慢响应 20 秒截止断言均失败。
- 新实现使用可取消异步请求，并由 Service 自己维护独立的 15 秒 watchdog；取消或超时会失效旧 generation，旧回调不能覆盖新连接或安排多余轮询。超时后保留真实故障状态并自动重试，不将故障伪装为健康。
- 仅设置 HTTP 库整体超时，在本机 Android 模拟器的慢速响应体反例中仍未按期结束，因此没有把库配置当作已验证行为；独立 watchdog 加入后反例通过。临时诊断日志已删除，不保留请求地址、密码或响应正文。
- 前端健康检查增加请求序号及卸载保护，修复旧失败晚于重试成功返回时重新显示异常的竞态；鉴权、超时、DNS、TLS、HTTP 和桥接错误分别提示。
- 红米真机没有连接 ADB，无法证明手机首次监控请求失败的网络或系统原因。上述是已复现的恢复缺陷，不以模拟器结果替代原手机根因证明。

### 验证结果

- 最终 TypeScript / Vitest 全量检查 **68 文件 / 1008 项通过**，在 2 GB heap、单 worker 条件下执行；Web E2E **61 通过 / 9 个既有平台跳过**；Java 单测 **21 项通过**。
- Android 模拟器监控与通知 **5 项通过**，覆盖慢速响应、自动恢复、手动重试、切换连接及旧响应、401/503/302、非法或超限响应、通知投递。监控 4 项另一次完整复跑也通过。
- 通知测试初次快速连续触发被 Android 的通知速率限制丢弃；仅测试夹具的加速轮询间隔改为 750 ms，保留全部通知断言，没有改变生产通知逻辑或削弱测试断言。
- 使用模拟器已保存的本机连接进行两次独立原生联调，每次均完成两轮真实状态检查，健康且错误清空。连接与密钥只在进程内使用，未输出或修改保存的连接；不是 Web HTTP 模拟。
- 独立源码审查通过，包括 watchdog 所有权、请求取消、旧 generation 隔离和响应关闭后的单次完成处理。
- Mac 锁屏后 Computer Use 无法继续页面操作，本轮未补验锁屏后的真实页面点击；红米真机私网及厂商后台策略仍未复测，既有图片问题也未修复或改标通过。

### 正式发布与资产核对

- [v0.5.33](https://github.com/cnwenf/codex-remote/releases/tag/v0.5.33) 于北京时间 **14:00** 公开发布。[流水线 34316791992](https://github.com/cnwenf/codex-remote/actions/runs/34316791992) 的 verify、Android、Mac ARM64、Mac x86_64、iOS、publish 全部成功。
- 全部 11 项正式资产重新下载；5 个二进制的 SHA-256、两个 DMG 的 `hdiutil verify`、三个移动归档的完整性检查均通过。
- 正式 APK 为 **0.5.33 / 42**，固定证书 SHA-256 `b931d267ce083976c506c1d5e2dd37c901152f5d4cd26e2da2bcff0e32605ece` 验证通过。Android 和两种 iOS 归档的 6 个 assets 文件及 `index.html` 全部逐字节匹配测试后的 dist；桥日志配置均为 none。
- iOS 模拟器与未签名实体 IPA 均为 **0.5.33 / 40**，模拟器签名校验通过；实体 IPA 仍需开发者签名。
- 两种 Mac App 的深度严格签名、App / Node / cloudflared 架构、版本及 Web 资源核对通过，Gateway bundle 在两种架构间逐字节相同。本机额外挂载自检未全部通过：一项退出 137，AMFI 日志记录临时签名／未知证书链拒绝；CI 中两个架构自检均通过。不将本机挂载执行算全部通过，本轮未安装 Mac 包或调整系统安全设置，临时挂载已卸载。
- GitHub latest、raw 备用地址和 CDN 清单均指向 0.5.33，固定 Android 下载提交为 `83c6de58029496f923aa9ebc03a8979d894c1462`。独立下载的 CDN APK 与 GitHub APK 的 SHA-256 完全一致。

| 0.5.33 正式资产 | SHA-256 |
| --- | --- |
| Android ARM64 APK | `d78ba82dd9f5ce715df265550581758e18404c4ba425e96d419e23dfeeddd529` |
| Mac ARM64 DMG | `17483b48495ff81264128bdfc562f2ecf9aee9f2a7fbbafaaa87c6ec8496886f` |
| Mac x86_64 DMG | `e41bf2f95d4ed5bce84eb51c77a23bff58ad0f90be660322efb0e5cc0541f95d` |
| iOS Simulator ZIP | `373bfe131e0bdeb921a278bb9ff43f7337e3353b1111cb547a225738a4b29406` |
| iOS unsigned IPA | `b06e96444f70c4f3da0f8e8a323750f2eaa2ddbb49fbf3ccb43473609b58f644` |
