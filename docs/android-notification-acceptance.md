...
本轮处理通知恢复及品牌图标、双向图片传输副本 ≤1,000,000 bytes、单次用户消息重复。源码修复、完整自动化和 Android/iPhone 模拟器包构建已完成；9 月 9 日锁屏已解除，最终移动包已安装且 Android instrumentation 通过，但 Mac 候选执行门禁仍阻挡最终联调，尚未发布新版本。
...
下一步需先解决 Mac 候选包的正常执行授权，再安装最终 Gateway，完成通知点击深链、图片和消息跨端重连验收。解锁、分支 CI 与最终 Android instrumentation 已完成，不重复作为待办。当前不合入 main、不创建 v0.5.32 标签、不公开发布；不能用已通过的 Web/构建替代这些门禁。
