...
下一步在 Mac 再次解锁后，复核此次 Gateway 中断后的移动连接 / 消息，确认 iPhone 上翻及原问题提示体验；本轮双向图片、同文发送、跨端排队转引导、通知点击深链已实际通过，不重复作为未测项。Mac 正常安装、分支 CI 与最终 Android instrumentation 已完成；全部通过后按已获授权发布。
