...
- Additional acceptance fixes (2026-09-08, not yet published): task-list status now prefers known live state over delayed rollout summaries, resolving lifecycle conflicts with one metadata-only latest-turn read. Turn identity, event revisions and snapshot identity protect against late responses; no message history is loaded for status checks. Real Android/iPhone list transitions passed on a dedicated multi-turn conversation.
- A subsequent native 10 MiB upload exposed an incremental-history gap: the upload and model response succeeded, but clients that had loaded earlier complete history missed an intervening image record outside the new tail page. The Gateway now reports the exact page byte range. A small client coverage module merges intervals and fills only holes between already-read ranges, one bounded page at a time, with a five-second deadline and eight-page automatic budget. Remaining gaps require an explicit continuation action. It preserves older-history cursors, uses left/right message anchors for insertion, ignores historical-tool replays for ordering, and cancels late recovery on back/archive/delete. Independent code review approved; final rebuilt-package native verification is still required. External mutation/truncation of source sessions is outside this append-only recovery contract.
- Acceptance boundary: an older dedicated test conversation returned a Codex execution-layer `Bad Request` without assistant/tool output on two models. The installed Gateway and send path were unchanged, and a fresh test conversation completed tools, a 10 MiB image, and a subsequent native follow-up. The older failure remains recorded as unexplained upstream behavior, not a Remote fix or a passed test.
- Final local candidate gate (2026-09-08): 856 tests across 63 files passed in an isolated single-worker run; final Web E2E passed 55 with the same nine platform skips. A concurrent build/E2E/check attempt timed out two existing tests; its log is retained, no assertions or timeouts were weakened. All seven installed Web assets match on macOS/Android/iPhone. Android sent another 10 MiB image after all clients had loaded the older history: Web and iPhone automatically restored it without refreshing, the native preview and final reply passed, both large-image hashes match, and five complete QA/question-index identities agree with the installed Gateway. Desktop remained running and saved connections/configuration were preserved. Formal release assets and post-publication installation remain the final gate.
