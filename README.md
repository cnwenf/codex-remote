...
阅读较长回答时，对话顶部会显示当前所读回答对应的原始问题；向上阅读旧轮次时，置顶内容随之切换。问题由 Mac 端按轮次和消息身份定位，不要求手机先加载全部历史。点开可展开长问题并分段继续读取；纯图片问题显示图片数，来自其他任务的输入标为委派问题。排队但尚未确认的输入不会替换当前问题。

首次打开很大的 session 时，问题索引会在后台分块建立，消息历史仍按页加载；暂未定位或无法确定关联时会明确提示，不用其他轮次的问题代替。
