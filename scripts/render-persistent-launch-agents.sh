...
insert_argument "$DESKTOP_PLIST" 0 "$APP_PATH/Contents/MacOS/ChatGPT"
insert_argument "$DESKTOP_PLIST" 1 "--remote-debugging-address=127.0.0.1"
insert_argument "$DESKTOP_PLIST" 2 "--remote-debugging-port=$CDP_PORT"
plutil -insert RunAtLoad -bool YES "$DESKTOP_PLIST"
plutil -insert KeepAlive -bool YES "$DESKTOP_PLIST"
